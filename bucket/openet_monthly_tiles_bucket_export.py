...
def main(
    ...
    """Export OpenET Monthly Tiles to a Google Cloud Storage Bucket
    ...
    # Assuming the bucket is in the same cloud project as GEE
    ...
    # Precompute the clipped output extent for each MGRS tile intersecting the study area
    logging.info('\nPrecomputing MGRS tile extents')
    mgrs_ftr_coll = ee.FeatureCollection(f'projects/openet/assets/mgrs/{region}/zones')
    if study_area_geom:
        mgrs_ftr_coll = mgrs_ftr_coll.filterBounds(study_area_geom)
    if mgrs_tiles:
        mgrs_ftr_coll = mgrs_ftr_coll.filter(ee.Filter.inList('mgrs', mgrs_tiles))
    mgrs_ftr_coll_info = mgrs_ftr_coll.getInfo()
    mgrs_extents = {
        ftr['properties']['mgrs']: [
            ftr['properties']['xmin'], ftr['properties']['ymin'],
            ftr['properties']['xmax'], ftr['properties']['ymax']
        ]
        for ftr in mgrs_ftr_coll_info['features']
    }

    mgrs_export_info = {}
    for mgrs_tile, mgrs_extent in mgrs_extents.items():
        logging.debug(f'\n{mgrs_tile}')
        logging.debug(f'  MGRS default extent: {mgrs_extent}')

        # Get the study area bounds in the MGRS coordinate system
        mgrs_crs = f'EPSG:326{mgrs_tile[:2]}'
        study_area_bounds = study_area_geom.bounds(maxError=1, proj=mgrs_crs).coordinates().get(0).getInfo()

        # Rounding probably isn't needed
        study_area_extent = [
            min([round(x[0], 0) for x in study_area_bounds]),
            min([round(x[1], 0) for x in study_area_bounds]),
            max([round(x[0], 0) for x in study_area_bounds]),
            max([round(x[1], 0) for x in study_area_bounds]),
        ]
        logging.debug(f'  Projected study area extent: {study_area_extent}')

        # Include 10 extra buffer cells when padding the extent
        cs = cellsize * 10
        ...
        logging.debug(f'  Snapped study area extent: {study_area_extent}')

        # Limit the projected study area extent to the MGRS extent
        study_area_extent[0] = max(study_area_extent[0], mgrs_extent[0])
        study_area_extent[1] = max(study_area_extent[1], mgrs_extent[1])
        study_area_extent[2] = min(study_area_extent[2], mgrs_extent[2])
        study_area_extent[3] = min(study_area_extent[3], mgrs_extent[3])
        logging.debug(f'  Limited study area extent: {study_area_extent}')

        # Compute the export geo transform and shape
        crs_transform = [cellsize, 0, study_area_extent[0], 0, -cellsize, study_area_extent[3]]
        shape_2d = [
            abs(int((study_area_extent[2] - study_area_extent[0]) / cellsize)),
            abs(int((study_area_extent[3] - study_area_extent[1]) / cellsize))
        ]
        logging.debug(f'  Transform: {crs_transform}')
        logging.debug(f'  Shape:     {shape_2d}')

        # TODO: Add check to make sure the shape is value and greater than 0

        mgrs_export_info[mgrs_tile] = {
            'crs': mgrs_crs,
            'crs_transform': crs_transform,
            'dimensions': shape_2d,
            # 'crs_transform': '[' + ','.join(map(str, crs_transform)) + ']',
    ...
    for iter_start_date, iter_end_date in iter_dates:
        ...
        for month_img_id in month_image_list:
            ...
            # Use the clipped MGRS tile extents for the output
            export_params['crsTransform'] = mgrs_export_info[mgrs_tile]['crs_transform']
            export_params['dimensions'] = mgrs_export_info[mgrs_tile]['dimensions']

            # # CGM - This will export the full tile extent
            ...
            export_name = tif_name_fmt.format(date=iter_start_dt.strftime('%Y%m%d'), mgrs=mgrs_tile)
            logging.debug(f'  {export_name}')

            description = export_name.replace('.tif', '').replace('/', '_') + '_bucket_export'
            logging.debug(f'  {description}')

            if bucket_folder:
                bucket_tif = f'{bucket_folder}/{export_name}'
                bucket_json = bucket_tif.replace('.tif', '.json')
            else:
                bucket_tif = f'{export_name}'
                bucket_json = bucket_tif.replace('.tif', '.json')
            logging.debug(f'  {bucket_tif}')
            ...
            task = ee.batch.Export.image.toCloudStorage(
                ...
                bucket=bucket_name,
                fileNamePrefix=bucket_tif.replace('.tif', ''),
                **export_params,
            )
            task.start()

            # Write the properties JSON file to the bucket
            ...
            if export_properties_json:
                ...
                blob = export_bucket.blob(bucket_json)
                ...
                # src_properties = img_info['properties']
                # for k in ['system:index', 'system:time_start']:
                #     if k in src_properties:
                #         del src_properties[k]
                # for k, v in src_properties.items():
                #     if k not in properties:
                #         properties[k] = v
                # TODO: Maybe move before task start and skip task start if
                #   this upload fails multiple times?
                max_retries = 4
                for i in range(1, 4):
                    try:
                        blob.upload_from_string(json.dumps(img_info['properties']))
...
def arg_parse():
    ...
    parser.add_argument(
        '--folder', default='', help='Bucket sub-folder')
    parser.add_argument(
        '--mgrs', default=[], nargs='+',
